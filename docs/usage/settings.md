...
Settings are defined as classes, which contain the parameters as properties. The classes are marked with the `#[Settings]` attribute, which makes them manageable by the settings-bundle. Besides the attribute, the class is just a normal PHP class, which can contain any kind of methods and properties.
...
Settings classes *must not* be final or contain final properties or methods as then no lazy loading proxy classes can be generated for them.
...
For better organization of related settings, you can embed settings classes into other settings classes. This is done by defining a property of the embedded settings class and marking it with the `#[EmbeddedSettings]` attribute. The Settings Manager automatically injects the (lazy loaded) embedded settings instance into the property, so that it can be used like a normal settings instance.
...
The `#[Settings]` attribute marks a class as a settings class. It is required for all settings classes.
...
* `storageAdapter` (optional): The class name of the storageAdapter service that should be used to store the settings (e.g. `InMemoryStorageAdapter::class`). If none is set, the global configured storage adapter will be used.
...
The `#[SettingsParameter]` attribute marks a property as a settings parameter. It is required for all properties, which should be managed by the settings-bundle.
...
* `label` (optional): A string or translation key, that can be used as a user-friendly label for the parameter, when showing it to user (e.g. in forms). This should be just a few words maximum.
* `description` (optional): A string or translation key, which can be used as a user-friendly description for the parameter, when showing it to user (e.g. in forms). Unlike the label, this can be a longer text giving more information about the parameter.
